...
module {
    ...
    public class Use<K, V>({db; obs=_; store; compare; key=_; regenerate=_; keep=_}: Config<K, V>) {
    ...
    public func Subscribe<K, V>({db; obs; store; compare; key; regenerate; keep}: Config<K, V>) : () {
        ...
        var _insert_unsubscribe = obs.index_insert.subscribe({
            next = insert;
            complete = O.null_func;
        });

        var _update_unsubscribe = obs.index_update.subscribe({
            next = update;
            complete = O.null_func;
        });

        var _delete_unsubscribe = obs.index_delete.subscribe({
            next = delete;
            complete = O.null_func;
        });
